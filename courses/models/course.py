...
class Course( RoutablePageMixin, TranslatedPage, BodyMixin  ):
    '''Implements a specific course.
    
    In particular, it implements the registration process for a course as well as data sharing.
    '''
    ...
    parent_page_types = [ 'courses.CourseInformationPage' ]
    ...
    def is_bookable( self ):
        '''returns if this course is bookable via the website''' 
        return self._get_me_or_parent().register_via_website 

    @property
    def has_data_sharing( self ):
        '''returns if this course has data sharing enabled'''
        return self._get_me_or_parent().share_data_via_website

    @property
    def is_fully_booked( self ):
        '''returns if the course is fully booked (that is, the number of registered 
        attendees is equal (or greater) than the number of maximum attendees.
        '''
    ...
    def registered_attendees_stats( self ):
        '''
        Returns a dict providing an overview of the the registered attendees
        
        *keys* of the dict are made of the attenndee's classname `display_name` 
        (or `display_name_plural`, if available). If multiple attendee classes share the 
        same display name, a number is appended.
        
        *values* of the dict are the sum of 
           - the number of attendees registered and validated for the course  
           - the number of attendees registered (but not valideted) for the course within the last week
           - the number of attendees from the waiting list for the course that have been asked (within the last week) to participated, but who did not respond yet
        '''
        attendees = {}
    ...
    def clean( self ):
        '''Automatically called during `save()`. Adds title and slug.''' 
    ...
    def started_in_past( self ):
        '''returns whether the start of this course is in the past.''' 
        return self.start <= datetime.date.today()

    def get_attendee_types( self ):
        '''
        returns the `attendee_types` for this course. Use this instead of the property 
        `attendee_types` since it respects the `overrule_parent` setting.
        '''
        return self._get_me_or_parent().attendee_types
    def get_max_attendees( self ):
        '''
        returns `max_attendees` and respects `overrule_parent`
        '''
    ...
    def get_price( self, attendee ):
        '''
        returns the price for `attendee`
        '''
    ...
    def has_waitlist_for( self, attendee):
        '''
        returns whether this course has a waitlist for `attendee`.
        '''
    ...
    def get_free_slots( self, Attendee ):
        '''
        returns the number of free slots for `Attendee`
        '''
    ...
    def get_n_attendees( self ):
        '''
        returns the total number of attendees based on `registered_attendees_stats`
        '''
        c = 0
    ...
    def get_context(self, request, *args, **kwargs):
        '''
        populates the context for rendering the page of this course. See wagtail/django docs.
        '''
    ...
    def _get_me_or_parent( self ):
        '''
        Helper function that returns the `parent` object (a course definition) or `self`,
        depending on whether `overrule_parent` is set.
        '''
